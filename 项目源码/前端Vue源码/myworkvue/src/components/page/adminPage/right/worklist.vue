...
<script>
    ...
    import Editwork from "../../common/editwork"
    import Exportexcel from "../../common/exportexcel"
    import YDetail from "../../common/yjdetail"
    import NDetail from "../../common/njdetail"
    import Workdetail from  '../../common/workdetail'

    export default {
      name: "right",
      components: {Addwork,Editwork,Exportexcel,YDetail,NDetail,Workdetail},
      inject:['reload'],
      ...
      data() {
        ...
        mounted: function () {
          ...
          // 第一页查询
          this.$axios.post('/findAllWorkPart','page=0').then(successResponse => {
          ...
          // 总数量
          this.$axios.get('/findAllWorkPartCount').then(successResponse => {
            if (successResponse && successResponse.status === 200) {
        ...
        methods:{
          ...
          handleCurrentChange(val) {
            if(this.workany.trim()==""){
              ...
              // 正常查询
              this.$axios.post('/findAllWorkPart','page='+(val-1)*8).then(successResponse => {
                if (successResponse && successResponse.status === 200) {
                  this.workList = successResponse.data
                }
              })
              ...
              // 模糊查询
              this.$axios.post('/findWorkSearch','workany='+this.workany+'&page='+(val-1)*8).then(successResponse => {
                if (successResponse && successResponse.status === 200) {
                  this.workList = successResponse.data
                }
              })
          ...
          search(){
            ...
            // 数量
            this.$axios.post('/findWorkSearchCount','workany='+this.workany).then(successResponse => {
              if (successResponse && successResponse.status === 200) {
                this.pageAll = successResponse.data
              }
            })
            ...
            // 详情
            this.$axios.post('/findWorkSearch','workany='+this.workany+'&page=0').then(successResponse => {
              if (successResponse && successResponse.status === 200) {
                this.workList = successResponse.data
              }
            })
